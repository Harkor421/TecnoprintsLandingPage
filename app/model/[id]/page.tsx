...
export async function generateMetadata({ params }: { params: { id: string } }): Promise<Metadata> {
  try {
    const res = await fetch(`${API_BASE}/api/models/${params.id}`, { cache: 'revalidate' })
    if (!res.ok) throw new Error('Failed to load')
    const model = await res.json()

    return {
      title: `${model.title} | Tecnoprints 3D`,
      description: model.description || `Modelo 3D: ${model.title}. Cotiza y imprime con Tecnoprints en Barranquilla.`,
      openGraph: {
        title: model.title,
        description: model.description || `Modelo 3D listo para imprimir`,
        type: 'website',
        url: `https://tecnoprints.com/${params.id}`,
        images: [
          {
            url: model.cover,
            width: 1200,
    ...
    return {
      ...
      description: 'Cotiza y imprime modelos 3D en Barranquilla',
    }
  }
}
